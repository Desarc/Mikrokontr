...
void play_sound(int code) {
	...
	if (code == ONE_LESS) activate_sound(one_less, one_less_size);
	else if (code == ONE_MORE) activate_sound(one_more, one_more_size);
	else if (code == HIT_WALL) activate_sound(hit_wall, hit_wall_size);
	else if (code == MUSIC) {
...
void map_shared_memory(void) {
	...
	/* data is next fragment, victory_pos, intro_pos, music_pos, active_sound_size (decomposed into bytes), looping, */
	char counters[15] = {next_bytes[0],next_bytes[1],victory_bytes[0],victory_bytes[1],intro_bytes[0],intro_bytes[1],music_bytes[0],music_bytes[1],sound_size_bytes[0],sound_size_bytes[1],sound_size_bytes[2],sound_size_bytes[3],frag_size_bytes[0],frag_size_bytes[1],0};
	size_t length_counters = 15;
	int fd_counters = -1;
...
void map_pointers(char *counters_addr) {
	...
	victory_pos = counters_addr+2;
	intro_pos = counters_addr+4;
	music_pos = counters_addr+6;
	active_sound_size = counters_addr+8;
	frag_size = counters_addr+12;
	looping_ptr = counters_addr+14;
}
